...
//		GameObject.DestroyImmediate(floor);
		...
		public void GenerateRooms (ref List<Room> _rooms, QuadTree _quadTree)
		{
				...
				if (_quadTree.northWest == null && _quadTree.northEast == null && _quadTree.southWest == null && _quadTree.southEast == null) {
						Room room = GenerateRoom (_quadTree);

						//call random function. if successful, call GenerateEnemy(room)
						int monsterType = Random.Range (0, 2);
						if (monsterType == 1) {
								int r = Random.Range (0, 100);
								if (r > 30) {
										GenerateEnemy (room, "bat");
								}
						} else {
								int r = Random.Range (0, 100);
								if (r > 70) {
										GenerateEnemy (room, "zombie");
								}
						}
						_rooms.Add (room);
		...
		{
				return tiles [row, col].id == Tile.TILE_EMPTY;
		}
	
		public bool IsPassable (int row, int col)
		{
				return tiles [row, col].id == Tile.TILE_ROOM || tiles [row, col].id == Tile.TILE_CORRIDOR;
		}
		...
		public void DigRoom (int row_bottom, int col_left, int row_top, int col_right)
		{
				...
				if (row_top > MAP_HEIGHT - 1)
						return;
				if (row_bottom < 0)
						return;
				if (col_right > MAP_WIDTH - 1)
						return;
				if (col_left < 0)
						return;
